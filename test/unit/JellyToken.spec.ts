...
describe('JellyToken', () => {
	...
	const initialBalance = ethers.utils.parseEther('133000000');
	const initialSupply = initialBalance.mul(BigNumber.from(3));
	...
	describe('Mint', () => {
		it('Mints tokens for an address when the sender has MINTER_ROLE', async () => {
			const balanceBeforeMint = await jellyToken.balanceOf(
	...
	describe('Burn', () => {
		...
		it('Allows users to burn their tokens', async () => {
			const balanceBeforeBurn = await jellyToken.balanceOf(
				otherAccount.address
			);
		...
		it('Decreases total supply', async () => {
			...
			expect(supplyBeforeBurn).to.eq(initialSupply.add(BigInt(100)));
